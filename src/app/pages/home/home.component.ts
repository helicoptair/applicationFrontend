...
export class HomeComponent implements OnInit {
  ...
  constructor(public settingsService: SettingsService, public appService: AppService, public voosService: VoosService, public mediaObserver: MediaObserver) {
  ...
  public obterVoosHomepage() {
    this.voosService.obterVoosHomepage().subscribe(data => {
      ...
      if (this.voos && this.voos.length > 0) {
        this.voos = this.voos.concat(result.data);
      }
      else {
      ...
      if (this.voos.length == this.pagination.total) {
        this.settings.loadMore.complete = true;
        this.settings.loadMore.result = this.voos.length;
      }
      else {
        this.settings.loadMore.complete = false;
      }
