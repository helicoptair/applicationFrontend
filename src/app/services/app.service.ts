...
export class AppService {
  ...
  public filterData(data: any, params: any, sort?: any, page?: any, perPage?: any) {
    ...
    if (params) {
      ...
      // Se filtrar pela quantidade de pax
      if (params.quantidade_pax) {
        data = data.filter((voo: Voos) => voo.quantidade_pax == params.quantidade_pax)
      }

      // Se filtrar pelo tempo de voo
      if (params.tempo_de_voo_minutos) {
        data = data.filter((voo: Voos) => voo.tempo_de_voo_minutos == params.tempo_de_voo_minutos)
      }

      // Se filtrar pela categoria
      if (params.categoria_de_voo) {
        data = data.filter((voo: Voos) => voo.categoria_de_voo == params.categoria_de_voo)
      }

      // Se filtrar pelo tipo de voo
      if (params.tipo_de_voo) {
        data = data.filter((voo: Voos) => voo.tipo_de_voo == params.tipo_de_voo)
      }
      ...
      if (params.price) {
        ...
        if (this.settingsService.settings.currency == 'BRL') {
          if (params.price.from) {
            data = data.filter((voo: Voos) => {
              if (voo.preco_pix_pessoa >= params.price.from) {
                return true;
              }
              if (voo.preco_pix_pessoa >= params.price.from) {
                return true;
          ...
          if (params.price.to) {
            data = data.filter((voo: Voos) => {
              if (voo.preco_pix_pessoa <= params.price.to) {
                return true;
              }
              if (voo.preco_pix_pessoa <= params.price.to) {
                return true;
  ...
  public sortData(sort: any, data: any) {
    if (sort) {
      switch (sort) {
        ...
        // EU FAZENDO MERDA
        case 'Preço Crescente (Total)':
          if (this.settingsService.settings.currency == 'BRL') {
            data = data.sort((a: any, b: any) => {
              if (a.preco_pix_total > b.preco_pix_total) {
                return 1;
              }
              if (a.preco_pix_total < b.preco_pix_total) {
                return -1;
          ...
          break;
          case 'Preço Crescente (Pessoa)':
          if (this.settingsService.settings.currency == 'BRL') {
            data = data.sort((a: any, b: any) => {
              if (a.preco_pix_pessoa > b.preco_pix_pessoa) {
                return 1;
              }
              if (a.preco_pix_pessoa < b.preco_pix_pessoa) {
                return -1;
  ...
  public getClients(): Client[] {
    return [
      { name: 'anac', image: 'images/clients/anac_logo.png' },
      { name: 'getyourguide', image: 'images/clients/getyourguide_logo.png' },
      { name: 'airbnb', image: 'images/clients/airbnb_logo.png' },
      { name: 'cadastur', image: 'images/clients/cadastur_logo.png' },
      { name: 'ultraplanna', image: 'images/clients/ultraplanna_logo.png' },
      { name: 'bluesky', image: 'images/clients/bluesky_logo.png' },
